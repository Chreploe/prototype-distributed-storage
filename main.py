...
def manual_event_input(args, starting_port, port_used, number_of_nodes):
    ...
    while True:
        ...
        if "k" in input_value:
        ...
        elif "w" in input_value:
            input_value = input_value[1:]
            event_parts = input_value.split()
            target_id = int(event_parts[0])
            message = event_parts[1]
            port = dict_id_port[target_id]
            
            print(f"Send message to node{target_id}, message: {message}")
            try: 
                node.send_db(message, port)
            except:
                print("ERROR")
        elif "o" in input_value:
            input_value = input_value[1:]
            event_parts = input_value.split()
            target_id = int(event_parts[0])
            message = event_parts[1]
            ...
            print(f"Read from node{target_id}, target index: {message}")
            try: 
                node.read_db(message, port)
            except:
                print("ERROR")
...
def main():
    ...
    for node_id in range(number_of_nodes):
        ...
        dict_id_port[node_id + 1] = starting_port + node_id
